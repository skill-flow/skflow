...
The `sh(cmd)` function SHALL execute the given shell command as a child process, capture stdout, stderr, and exit code, and record the call in the session log. In the compiled state machine, `sh()` is a yield point but the runtime SHALL auto-resume it without exiting the process.
...
- **WHEN** a shell command does not complete within 60 seconds (default)
